...
The Council selected a range of risk elements to be evaluated at either the managed species level, the species and sector level, or the ecosystem level. An overview of the risk elements with definitions and associated indicators as adopted by the MAFMC is presented below. In the following sections, we describe each risk element in more detail along with proposed definitions of low, low-moderate, moderate-high, and high risk. Indicators are then shown for each risk element and a preliminary risk categorization based on the indicator is presented. 

*These risk definitions are currently in draft form, pending Council review. The primary goal of presenting this information is to clearly define and reach agreement about risks of concern to the Council. All risk rankings shown can be further adjusted to meet Council needs. This is not a final risk assessment. A Mann-Kendall test for monotonic trends is used to test significance of both long term and recent trends.*
...
| *Economic* |  |
| Commercial Revenue   | Risk of not maximizing fishery value             | Revenue by fleet |
...
Species distribution on the NE Shelf can be characterized by the position in the ecosystem along an axis oriented from the southwest to the northeast, referred to as the along shelf distance, and by depth. Along shelf distances range from 0 to 1360, which relates to positions along the axis from the origin in the southwest to the northeast in kilometer units. The mean along shelf distance for several MAFMC species by year is shown below; most are consistent with the predictions of NEVA and show a northeastward change in distribution aside from shortfin squid (an offshore species incompletely sampled by the coastal NEFSC trawl survey).  Mean depth has not changed significantly for these species. Information for more species is available at http://www.nefsc.noaa.gov/ecosys/current-conditions/species-dist.html. 
...
##Offshore Habitat
This element is applied at the species level. The risk of achieving OY due to changes in offshore habitat quality and quantity can be assessed using trends derived from experimental species-specific habitat modeling. *In addition, the number of threats from other human uses can be enumerated, but this has yet to be included.*
...
<!--
```{r, fig.cap="Shifts in modeled species spring habitat area over time; A: Black sea bass, B: Summer flounder, C: Scup, D: Butterfish, E: Atlantic mackerel, F: Longfin squid, G: Shortfin squid, H: Dogfish, I: Goosefish  \\label{shifts}", fig.height=9, echo = F, message = FALSE, warning = FALSE}
#knitr::include_graphics(file.path(image.dir, 'dist-trend_all.png'))
#out.width='\\linewidth'
...
soe.plot(SOE.data, 'Time', 'BLACK SEA BASS Max Habitat Index Spring',    stacked = 'A', scale.axis = 100, 
         rel.y.num = 0.9)
soe.plot(SOE.data, 'Time', 'SUMMER FLOUNDER Max Habitat Index Spring',   stacked = 'B', scale.axis = 100,
         rel.y.num = 0.9)
soe.plot(SOE.data, 'Time', 'SCUP Max Habitat Index Spring',              stacked = 'C', scale.axis = 100,
         rel.y.num = 0.9)
soe.plot(SOE.data, 'Time', 'BUTTERFISH Max Habitat Index Spring',        stacked = 'D', scale.axis = 100,
         rel.y.num = 0.9)
soe.plot(SOE.data, 'Time', 'ATLANTIC MACKEREL Max Habitat Index Spring', stacked = 'E', scale.axis = 100,
         rel.y.num = 0.9)
soe.plot(SOE.data, 'Time', 'LONGFIN SQUID Max Habitat Index Spring',     stacked = 'F', scale.axis = 100,
         rel.y.num = 0.9)
soe.plot(SOE.data, 'Time', 'NORTHERN SHORTFIN SQUID Max Habitat Index Spring', stacked = 'G', 
         scale.axis = 100, rel.y.num = 0.9)
soe.plot(SOE.data, 'Time', 'SPINY DOGFISH Max Habitat Index Spring', stacked = 'H', 
         scale.axis = 100, rel.y.num = 0.9)
soe.plot(SOE.data, 'Time', 'GOOSEFISH Max Habitat Index Spring', stacked = 'I', 
         scale.axis = 100, rel.y.num = 0.9)
...
This experimental habitat index is still being studied and improved, so  habitat risk rankings based on this can be considered preliminary. Overall, black sea bass, summer flounder, scup, and spiny dogfish have long term increasing trends in offshore habitat, and Atlantic mackerel and longfin squid have short term increasing trends. Therefore, these species rank low risk for this element. However, butterfish, shortfin squid, and goosefish have long term decreasing trends in offshore habitat, and shortfin squid have a short term decreasing trend as well. Therefore, thse species rank moderate-high risk (butterfish, goosefish) and high risk (shortfin squid) for this element. Ocean quahogs, surfclams, tilefish, and bluefish are not adequately sampled by the bottom trawl survey and were not included in this analysis, similar to unmanaged forage and deepsea corals. 
...
This is aggregate commercial revenue for MAFMC managed species. There is a long term increase in revenue, but a recent significant decrease in revenue, indicating high risk to commercial fishery profit. This trend is consistent with the trend first shown in the EAFM Interactions white paper and published in @gaichas_framework_2016 (Figs 2-3).
...
soe.plot(allrev, 'Time', 'MAFMC Revenue', x.label="Year", y.label="Commercial Revenue 2015 Adjusted USD", rel.y.num = 1.1, rel.y.text = 1)
```
...
This index shows a significant long term decrease, which would suggest a moderate-high risk to fishery business resilience based on diversity in species revenue. 
...
The number of shoreside support companies that include seafood merchant wholesalers, seafood product preparation and packaging, and seafood markets across all Mid-Atlantic states shows a long term decrease, which on its own would represent moderate-high risk to fishery resilience. However, the number of non-employer entities which include seafood preparation and packaging and seafood markets shows both a long term and short term increase. Trends in other shoreside fishery supporting businesses such as gear manufacturers and welding companies are not included here due to aggregation of the statistics. Therefore, until further information is available, we rate this low-moderate risk due to the conflicting trends of declining companies and increasing non-employer entities. 
...
##Control
This element is applied at the species and sector (commercial/recreational) level. This element addresses the level of management control in terms of catch measurement and monitoring. Adequate management control indicates low risk of not achieving OY, while poor management control indicates higher risk of not achieving OY. Actual catch is compared with the specified ABC over the most recent five years of fishery history. 
...
##Other Ocean Uses
This element is applied at the species and sector (commercial/recreational) level. This element addresses the risk of fishery displacement or damage of a fishery resource and/or habitat that supports it as a result of non-fishing activities in the ocean.  
...
This element is applied at the species and sector (commercial/recreational) level. Constituents have frequently raised concerns about the complexity of fishery regulations and the need to simplify them to improve their efficacy. Complex regulations may lead to non-compliance and/or impact other fisheries. 
...
##Discards
This element is applied at the species and sector (commercial/recreational) level. Stakeholders have identified the reduction of discards as a high priority in the Council management program, especially those caused by regulations since they represent biological and economic waste.  Discards of either the target or non-target species in the fishery would be taken into consideration. 
...
##Allocation
This element is applied at the species and sector (commercial/recreational) level. This element addresses the risk of not achieving OY due to spatial mismatch of stocks and management allocations. Indicators for difficulty of allocation include a combination of distribution shifts (see above) and the number of interests (sectors, states, etc.) requiring allocation.
