...
## Long-term distribution shifts (MAB)

```{r 'species_dist_kde_MAB' , echo = F, out.width='10cm', out.height='10cm', fig.show='hold'}
...
## Long-term distribution shifts (NE)

```{r 'species_dist_kde_NE' , echo = F, out.width='10cm', out.height='10cm', fig.show='hold'}
...
Vince's figures here

## Condition factor

Laurel's new figures here
...
soe.plot(SOE.data.2018, 'Time', "right whale total",            
         scale.axis = 1, rel.y.num = 1.2, stacked = NA, endshade = F,status = F,
         suppressAxis = F, full.trend = F, lwd = 1.5, point.cex = 1, end.start = 2005)
soe.stacked.axis('Year', expression('Total Right Whale Abundance, n'), outer = F)
lw_CI <- SOE.data.2018[SOE.data.2018$Var == 'right whale total lower 95',]$Value
up_CI <- SOE.data.2018[SOE.data.2018$Var == 'right whale total upper 95',]$Value
year <- SOE.data.2018[SOE.data.2018$Var == 'right whale total upper 95',]$Time
...
```{r rw_abundance_mf, echo=F, fig.align='center', fig.height=5, fig.width=7, message=FALSE, warning=FALSE, include=T, out.width='1\\linewidth'}
opar <- par(mfrow = c(2, 1), mar = c(0, 0, 0, 0), oma = c(4, 6, 2, 6))
