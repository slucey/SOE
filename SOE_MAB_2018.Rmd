...
soe.stacked.axis("Time",expression("Landings, 10"^3*"metric tons"), outer = F, rel.x.text = 1, rel.y.text = 1)
```
...
soe.plot(SOE.data.2018, "Time", "Recreational Seafood MA", stacked = NA, status = F, full.trend = T, rel.y.num = 1.1, 
...
soe.plot(SOE.data.2018, "Time","Mid-Atlantic average fleet diversity", stacked = "A",
         endshade = T, rel.y.num = 1, full.trend = F, status = F,end.start = 2007)
...
soe.plot(SOE.data.2018, "Time", "chesapeake bay HAB", stacked = NA,status = F,mean_line = F,
         endshade = F, rel.y.num = 1, full.trend = F, scale.axis = 1,end.start = 2007,
...
\btwocol
```{r 'commercial_reliance_and_vulnerability MAB' , fig.cap="Commercial reliance and social vulnerability (MAB) \\label{commvulmap}",echo = F, message=F, include=T, warning=F, fig.width=5, fig.height=7, fig.align = 'center'}
...
```{r 'community_reliance_and_social_vulnerability MAB' , fig.cap="Recreational reliance and social vulnerability (MAB) \\label{recvulmap}", echo = F, message=F, include=T, warning=F, fig.width=5, fig.height=7, fig.align = 'center'}
...
```{r occupancy_MAB, fig.cap="Summer flounder (left) and black sea bass (right) modeled shelf habitat trends (Top row: Spring, bottom row: Fall)", out.width=c('9.5cm','9.5cm'), out.height=c('5cm','5cm'), fig.align='center', fig.show='hold',fig.ncol = 2,warning=F, echo=F}
opar <- par(mfrow = c(2, 1), mar = c(0, 0, 0, 0), oma = c(3.5, 5, 2, 4))
...
```{r southern kingfish, fig.cap="Southern kingfish occurrences in the observer database", echo=F, message=FALSE, warning=FALSE ,fig.align='center', fig.height=6, fig.width=6}
if(map_figs){
