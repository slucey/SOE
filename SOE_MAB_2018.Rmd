...
price_ind<-readPNG("price_ind_MAB.png")
vol_ind<-readPNG("vol_ind_MAB.png")
...
```{r bsb_maps,fig.cap="Black sea bass historical and current abundance estimates (A), current thermal habitat estimate (B), and 20-40 year thermal habitat projection (C). \\label{dists}" , fig.width = 8,fig.height = 2.75, fig.show='hold',fig.pos='H'}
#read images
kde <-readPNG("black_sea_bass_kde.png")
contemp <-readPNG("bsb_contemp.png")
proj <- readPNG("bsb_20_40.png")

par(mar=c(0,0,0,0), xpd=NA, mgp=c(0,0,0), oma=c(0,0,0,0), ann=F, mfrow = c(1,3))
plot.new()
usr<-par("usr")

#fill plot with images
rasterImage(kde, usr[1], usr[3], usr[2], usr[4])
rasterImage(contemp, usr[1]+1.1, usr[3], usr[2]+1.1, usr[4])
rasterImage(proj, usr[1]+2.1, usr[3], usr[2]+2.1, usr[4])

#Add text
text(.12,.9, "A", cex=1.5)
text(1.2,.9, "B", cex=1.5)
text(2.2,.9, "C", cex=1.5)
text(2.04,.95,"Suitable \nthermal \nhabitat", cex = 0.65)
text(3.04,.95,"Suitable \nthermal \nhabitat", cex = 0.65)
...
```{r sf_maps,fig.cap="Summer flounder historical and current abundance estimates (A), current thermal habitat estimate (B), and 20-40 year thermal habitat projection (C). \\label{dists}" , fig.width = 8,fig.height = 2.75, fig.show='hold',fig.pos='H'}
#read images
kde <-readPNG("summer_flounder_kde.png")
contemp <-readPNG("summer_flounder_contemp.png")
proj <- readPNG("summer_flounder_20_40.png")
...
#read images
#read images
ppd <-readPNG('PPD_anom_MAB.png')
chl <-readPNG('CHL_anom_MAB.png')
par(mar=c(0,0,0,0), xpd=NA, mgp=c(0,0,0), oma=c(0,0,0,0), ann=F, mfrow = c(1,2))
plot.new()
usr<-par("usr")
...
soe.plot(SOE.data.2018, "Time", "Centropages typicus abundance anomaly MAB", 
         anomaly = T, stacked = "A", suppressAxis = T, x.start = 1986)
