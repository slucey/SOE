...
soe.plot <- function(data, x.var, y.var, x.label = '', y.label = '', tol = 0.1,
  ...
  if (status == T){
    ...
    #Add axis labels
    if(!is.na(stacked)) text(u[1], u[4], labels = stacked, cex = 1, adj = c(-0.5, 1.5))
    if(is.na(stacked)){
...
| E: Benthivores (Eat bottom dwellers)       |  25 | black sea bass, scup, tilefish, tautog, cunner, blue crab, red crab, lobster, ocean pout, haddock, yellowtail, winter, and witch flounders, barndoor skate, American plaice, other crabs|
...
**Table 3**: Proportion of landings and revenue derived from managed species in the Mid-Atlantic. 
...
```{r seafood_landings_MAB, fig.cap="MAFMC seafood specific landings (red) and total commericial landings (black). A: Apex predators, B: Piscivore, C: Planktivore, D: Benthivore, E: Benthos \\label{landings}",echo = F, warning = F, include = T, fig.asp=0.78, fig.align='center'}
...
```{r managed_landings , fig.cap="Total commercial seafood landings (black), MAFMC managed seafood landings (red)", echo=F, message=FALSE, warning=FALSE , fig.asp=0.35, fig.align = 'center'}
...
```{r rec_catch_MAB, fig.cap="Total recreational harvest  \\label{recfood}", echo = F, message=F, include=T, warning=F, fig.asp=0.35, fig.align = 'center'}
...
```{r managed_revenue , fig.cap="Total revenue by region and MAFMC managed species\\label{totcommrev}", echo = F, message=F, include=T, warning=F ,  fig.asp=0.35, fig.align = 'center'}
opar <- par(mar = c(4, 6, 2, 6))
...
```{r fleet diversity MAB, fig.cap="(A) Fleet diversity and (B) fleet count\\label{commdiv}", echo=F, message=FALSE, warning=FALSE, include=T, fig.asp=0.5, fig.align = 'center'}
...
soe.plot(SOE.data.2018, "Time", "Mid-Atlantic average fleet diversity", stacked = "A",
         rel.y.num = 0.9, end.start = 2008, tol = 0.15)
soe.plot(SOE.data.2018,"Time", "Mid-Atlantic fleet count", stacked = "B",
         rel.y.num = 0.9, end.start = 2008)
...
```{r revenue diversity MAB, fig.cap="Diversity in species revenue\\label{revdiv}", echo = F, message=F, include=T, warning=F,  fig.asp=0.35, fig.align = 'center'}
opar <- par(mar = c(4, 6, 2, 6))

soe.plot(SOE.data.2018, 'Time', "Mid-Atlantic commercial species diversity", 
         end.start = 2008, x.label = 'Year', y.label = 'Shannon Index', 
         rel.y.text = 1, rel.y.num = 1, y.line = 2.5, x.line = 2)
```
...
```{r recreation MAB, fig.cap="(A) Number of anglers and (B) number of trips", echo=F, message=FALSE, warning=FALSE, include=T,  fig.asp=0.5, fig.align = 'center'}
opar <- par(mfrow = c(2, 1), mar = c(0, 0, 0, 0), oma = c(4, 6, 2, 6))
...
```{r hab_MAB, fig.cap="Occurrence of algal blooms in Chesapeake Bay at cell concentrations greater than the NCCOS Phytoplankton Monitoring Network threshold for consideration as a bloom event (black), and occurrence of blooms at concentrations warranting action by the Virginia Department of Health (red).", echo = F, warning = F, include = T,fig.width=7, fig.height=2.5, fig.align='center'}
opar <- par(mar = c(4, 6, 2, 6))

soe.plot(SOE.data.2018, "Time", "chesapeake bay HAB", mean_line = F, rel.y.num = 1.1, 
         full.trend = F, end.start = NA, ymin = FALSE, y.lower = 0, extra = TRUE, 
         x.var2 = "Time", y.var2 = "HAB events at VDH threshold", mean_line.2 = F,
...
plot(coast, xlim = c(-77.5,-71), ylim = c(35,41),col = "grey")
...
#text
text(-73.3, 36.9, "Commercial Reliance",srt=90, cex = .85)
text(-72,35.8, "Social Vulnerability", cex = .85)
}
...
#text
text(-73.3, 36.9, "Recreational Reliance",srt=90, cex = .85)
text(-72,35.8, "Social Vulnerability", cex = .85)
}
...
```{r 'species_vulnerability_MAB' , fig.cap="Species vulnerability in Mid-Atlantic fishing communities \\label{sppvul}", echo = F, out.width = "450pt", fig.align='center', eval = T}
...
```{r harbor_porpoise, fig.cap="Harbor porpoise bycatch estimated compared with PBR \\label{harbporp}", echo = F, message=F, include=T, warning=F, fig.height=2.5, fig.align = 'center'}
opar <- par(mar = c(4, 6, 2, 6))
...
```{r rw_abundance, fig.cap="Right whale population estimate\\label{rightwh}",echo = F, message=F, include=T, warning=F, fig.height=2.5, fig.align = 'center'}
opar <- par(mar = c(4, 6, 2, 6))
...
```{r fall_survey_MAB, fig.cap="Fall (left) and Spring (right) MAB Survey Biomass (A: Piscivore, B: Planktivore, C: Benthivore, D: Benthos)", out.width=c('9.5cm','9.5cm'), out.height=c('14cm','14cm'), fig.show='hold',fig.ncol = 2, echo = F, warning = F, include = T}
...
```{r larval_diversity, fig.cap="Ichthyoplankton Shannon diversity and species counts in the Spring (Top row) and Fall (bottom row)", out.width=c('9.5cm','9.5cm'), out.height=c('5cm','5cm'), fig.show='hold',fig.align='center', fig.ncol = 2,warning=F, echo=F}
...
```{r, fig.cap="Black sea bass (KDE, contemporary, 20-40 year thermal habitat projection) \\label{dists}", fig.height=0.05}
opar<-par(mar=c(0,0,0,0))
plot(0,0,typ='n',axes=F,xlab='',ylab='')

```

```{r bsb_species_distributions_MAB, fig.cap="Black sea bass mean depth and along shelf distance trends (Top row: Spring, bottom row: Fall)", out.width=c('9.5cm','9.5cm'), out.height=c('5cm','5cm'), fig.show='hold',fig.align='center', fig.ncol = 2,warning=F, echo=F}
opar <- par(mfrow = c(2, 1), mar = c(0, 0, 0, 0), oma = c(3.5, 5, 2, 4))


soe.plot(SOE.data.2018, "Time", "black sea bass spring mean depth", stacked = "A",
         rel.y.num = 1.1, x.start = 1963, end.start = 2007)
soe.plot(SOE.data.2018, "Time", "black sea bass fall mean depth", stacked = "B",
         rel.y.num = 1.1, line.forward = T, end.start = 2007)
...
```{r, fig.cap="Summer flounder (KDE, contemporary, 20-40 year thermal habitat projection) \\label{dists}", fig.height=0.05}
opar<-par(mar=c(0,0,0,0))
plot(0,0,typ='n',axes=F,xlab='',ylab='')

```

```{r sf_species_distributions_MAB,fig.cap="Summer flounder mean depth and along shelf distance trends (Top row: Spring, bottom row: Fall)", out.width=c('9.5cm','9.5cm'), out.height=c('5cm','5cm'), fig.show='hold',fig.align='center',fig.ncol = 2,warning=F, echo=F}
opar <- par(mfrow = c(2, 1), mar = c(0, 0, 0, 0), oma = c(3.5, 5, 2, 4))


soe.plot(SOE.data.2018, "Time", "summer flounder spring mean depth", stacked = "A",
         rel.y.num = 1.1, x.start = 1963, end.start = 2007)
soe.plot(SOE.data.2018, "Time", "summer flounder fall mean depth", stacked = "B",
         rel.y.num = 1.1, line.forward = T, end.start = 2007)
soe.stacked.axis("Year", "Mean depth, m")
...
```{r occupancy_MAB, fig.cap="Summer flounder (left) and black sea bass (right) modeled shelf habitat trends (Top row: Spring, bottom row: Fall)", out.width=c('9.5cm','9.5cm'), out.height=c('5cm','5cm'), fig.show='hold',fig.align='center',fig.ncol = 2,warning=F, echo=F}
opar <- par(mfrow = c(2, 1), mar = c(0, 0, 0, 0), oma = c(3.5, 5, 2, 4))

soe.plot(SOE.data.2018, "Time", "summer flounder spring occupancy", stacked = "A",
         rel.y.num = 1.1, scale.axis = 10^3, end.start = 2007)
...
soe.plot(SOE.data.2018, "Time", "black sea bass spring occupancy", stacked = "C",
         rel.y.num = 1.1, scale.axis = 10^3, suppressAxis = T, end.start = 2007)
...
```{r long_term_sst,  echo=F, message=FALSE, warning=FALSE , fig.height=2.5, fig.align = 'center'}
opar <- par(mar = c(4, 6, 2, 6))
...
```{r, fig.cap="Sea surface temperature seasonal anomaly, 2017 \\label{SSTanommap}", fig.height=0.05}
opar<-par(mar=c(0,0,0,0))
plot(0,0,typ='n',axes=F,xlab='',ylab='')
...
```{r, fig.cap="Primary production and chlorophyll anomaly maps \\label{ppchlanom}", fig.height=0.05}
opar<-par(mar=c(0,0,0,0))
plot(0,0,typ='n',axes=F,xlab='',ylab='')

```

```{r, MAB zooplankton, echo = F, warning = F, include = T, fig.width=7, fig.height=5, fig.align='center', warning=F}
...
soe.plot(SOE.data.2018, "Time", "Centropages typicus abundance anomaly MAB", 
         anomaly = T, stacked = "B", suppressAxis = T, x.start = 1986)
...
```{r 'condition factor MAB' , fig.cap="Groundfish condition, MAB", echo = F, out.height='10cm', fig.align='center'}
