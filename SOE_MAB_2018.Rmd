...
#thermal habitat bathymetry
th <- getNOAA.bathy(lon1 = -77, lon2 = -65, lat1 = 35, lat2 = 45,
              resolution = 10) -> nesbath

#color palette for thermal habitat maps
...
#themal habitat map function
th_plot <- function(data, species, letter, z.max){
  data <- nc_open(data)
  ...
  #max value for breaks
  #z.max <- SOE.data.2018[grepl(paste(species, "max th"),SOE.data.2018$Var),]$Value
  #z.max <- as.numeric(z.max)
  ...
  plot(r, col=colors, ylim=c(35,45),
       ...
       zlim = c(0,z.max),
       legend.width = 1.5,
       axes=F, interpolate = F, las = 1,box = T,
       ...
       legend.args=list(text=expression(paste("     Thermal \n      Habitat")),
                        side=3, font=2, line=1, cex=.5))
  
  map("worldHires", xlim=c(-77,-65),ylim=c(35,45), fill=T,border=0,col="gray", add = T)
  map.axes(cex.axis = .7)
  
  plot(nesbath,deep=-200, shallow=-200, step=1,add=T,lwd=1.2,col="black",lty=3)
...
```{r seafood_landings_MAB, fig.cap="MAFMC seafood specific landings (red) and total commericial landings (black). A: Apex predators, B: Piscivore, C: Planktivore, D: Benthivore, E: Benthos \\label{landings}",echo = F, warning = F, include = T, fig.asp=0.78, fig.align='center',fig.pos='H'}
...
```{r 'Rev_Price_Vol_MAB', fig.cap="Bennet indicator, all species aggregated \\label{Bennet}", echo = F, out.width = "475pt", fig.align='center',fig.pos='H'}
...
```{r, mariculture MAB, echo = F, warning = F, include = T,fig.asp=0.5, fig.align='center',fig.pos='H'}
...
```{r black sea bass_maps, echo=FALSE, fig.cap="Atlantic cod historical and current abundance estimates (A), current thermal habitat estimate (B), and 20-40 year thermal habitat projection (C). \\label{dists}", fig.height=2.75, fig.pos='H', fig.show='hold',warning=F, message=F,  fig.width=8}
#read image
kde <-readPNG("black_sea_bass_kde.png")

par(mar=c(2.5,2.8,2,2), xpd=NA, mgp=c(3,1,0), oma=c(0,1,0,0), ann=F, mfrow = c(1,3))
plot.new()
usr<-par("usr")
...
#fill plot with images
rasterImage(kde, usr[1], usr[3], usr[2], usr[4])
text(.12,.9, "A", cex=1.5)
th_plot(data = "Black Sea Bassfall_2.nc", species = "black sea bass", letter = "B", z.max = 1.353462)
th_plot(data = "Black Sea Bassfall_4.nc", species = "black sea bass", letter = "C", z.max = 1.353462)

```


```{r bsb_species_distributions_MAB, fig.cap="Black sea bass mean depth and along shelf distance trends (Top row: Spring, bottom row: Fall)",  echo = F, fig.show='hold', fig.align='default', out.width='.49\\linewidth',warning = F, message = F,fig.pos='H'}
opar <- par(mfrow = c(2, 1), mar = c(0, 0, 0, 0), oma = c(3.5, 5, 2, 4))
...
```{r summer flounder_maps, echo=FALSE, fig.cap="Atlantic cod historical and current abundance estimates (A), current thermal habitat estimate (B), and 20-40 year thermal habitat projection (C). \\label{dists}", fig.height=2.75, fig.pos='H', fig.show='hold',warning=F, message=F,  fig.width=8}
#read image
kde <-readPNG("summer_flounder_kde.png")
...
rasterImage(kde, usr[1], usr[3], usr[2], usr[4])
text(.12,.9, "A", cex=1.5)
th_plot(data = "Summer Flounderfall_2.nc", species = "summer flounder", letter = "B", z.max = 2.37253)
th_plot(data = "Summer Flounderfall_4.nc", species = "summer flounder", letter = "C", z.max = 2.37253)

```

```{r sf_species_distributions_MAB,fig.cap="Summer flounder mean depth and along shelf distance trends (Top row: Spring, bottom row: Fall)",  echo = F, fig.show='hold', fig.align='default', out.width='.49\\linewidth',warning = F, message = F,fig.pos='H'}
opar <- par(mfrow = c(2, 1), mar = c(0, 0, 0, 0), oma = c(3.5, 5, 2, 4))
...
```{r occupancy_MAB, fig.cap="Summer flounder (left) and black sea bass (right) modeled shelf habitat trends (Top row: Spring, bottom row: Fall)", out.width=c('9.5cm','9.5cm'),  echo = F, fig.show='hold', fig.align='default', out.width='.49\\linewidth',warning = F, message = F,fig.pos='H'}
opar <- par(mfrow = c(2, 1), mar = c(0, 0, 0, 0), oma = c(3.5, 5, 2, 4))
...
```{r SST_PPD_CHL, fig.cap="SST (A), primary production (B), and Chl a (C) over 2017 (colored polygons) compared against long-term mean (black line) and +/- 1 standard deviation (grey polygon)", echo = F, out.width=c('6.5cm','6.5cm','6.5cm'), out.height=c('7cm','7cm','7cm'), fig.show='hold',fig.ncol = 3,fig.pos='H'}
...
if (map_figs == T){
SST_plot <- function(raster_data, text){
  d <- raster(raster_data, varname = "ANOM")
  e  <- extent(-78.5, -63, 33, 45)
  d <- crop(d, e)
  ...
  par(mar = c(2,4,3.2,1))  
  plot(d, col=colors(n=color_levels),xlim = c(-78,-63),ylim = c(34,44),
  ...
  contour(bathy,xlim = c(-78,-63),drawlabels = F,nlevels = 5,add=T)
  plot(coast, col = 'grey',add = T)
  
  axis(1, at = c(-75,-70,-65), labels = paste( c(-75,-70,-65) * -1, 'W'),col = NA, col.ticks = 1)
  axis(2, at = axTicks(2), labels = paste(axTicks(2), 'N'), las = T, pos = -78.71,col = NA, col.ticks = 1)
  
  abline(v = -78.69,lwd =2)
  arrows(-78.71,35,-64.55,35,angle = 180,lwd = 2.5)
  text(-77.75,43.5,text,cex = 1.5)
}


sst_files <- list("2017_Fall_SST_anomaly.nc","2017_Winter_SST_anomaly.nc",
                  "2017_Spring_SST_anomaly.nc","2017_Summer_SST_anomaly.nc")
letters <- list("A","B","C","D")

opar<-par(mfrow=c(2,2), oma = c(0,0,0,1))
for (i in 1:length(sst_files)){
  SST_plot(sst_files[[i]],letters[[i]])
}
...
```{r chl_ppd_maps,fig.cap="Primary production and chlorophyll anomaly maps \\label{ppchlanom}" ,  fig.width = 8,fig.height = 5.4, fig.show='hold',fig.pos='H'}
#read images
#read images
...
```{r 'condition factor MAB' , fig.cap="Groundfish condition, MAB", echo = F, out.height='10cm', fig.align='center',fig.pos='H'}
