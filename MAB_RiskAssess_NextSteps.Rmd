...
One potential prioritization approach focuses on the single species with the largest number of high risk rankings across all elements, which is black sea bass (8 high Risk Elements, Tables \ref{sptable}-\ref{ecotable}). A second approach focuses on the fishery with the highest landings value (as a proxy for seafood production and economic benefits), which is longfin squid, a species with uncertainties in assessment and potential for distribution shifts along with high risks identified across multiple management elements (Tables \ref{sptable}-\ref{spsectable}). A third approach focuses on the risk element with the most high risk categorizations across all categories, which is allocation (12 high risk species/sector combinations; Table \ref{spsectable}). These prioritization approaches use only the risk elements that could be ranked; inclusion of additional risk elements of interest to the Council (Table \ref{riskelpa}) may not change the selected approach but could change the outcome.
