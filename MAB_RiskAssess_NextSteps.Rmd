...
One potential approach focuses on the species with the largest number of high risk rankings across all elements, which is black sea bass (8 high Risk Elements, Tables \ref{sptable}-\ref{ecotable}). A second approach focuses on the species complex with the highest landings value (as a proxy for benefits), which is longfin squid, a species with uncertainties in assessment and potential for distribution shifts along with high risks identified across multiple management elements (Tables \ref{sptable}-\ref{spsectable}). A third approach focuses on the risk element with the most high risk categorizations across species, which is allocation (12 high risk species/sector combinations; Table \ref{spsectable}). 

An example conceptual model based on the first approach has been developed to illustrate the process (Fig. \ref{bsbex}).  This model was developed by modifying the existing Mid-Atlantic conceptual model using the risk assessment results tables to ensure that all key factors affecting black sea bass and their fisheries were considered. We can step through this process briefly during the EOP presentation to illustrate methods for building a model. While it looks complex, this need not be time consuming and the process can incorporate the knowledge of managers, fishery participants, and scientsts. 

A conceptual model based on the second approach was recently developed as well, which emphasizes habitat linkages affecting availability for longfin squid as well as economic and market factors affecting fishery effort (Fig. \ref{squidex}). Again, this is illustrative of an approach and not intended to be prescriptive. 
