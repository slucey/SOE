...
```{r, fig.cap="Gulf of Maine Ecosystem \\label{conceptual}", echo = F, out.width = "450pt", fig.align='center'}
knitr::include_graphics(file.path(image.dir, 'GBandGOM_simple.png'))

```


## Ecosystem and living marine resources status 
...
*The New England Council (NEFMC) is not meeting objectives at the managed species level** for fishing mortality (F) rates and biomass (B) levels relative to established reference points, with the exception of . This has been a focus of management. 

```{r, fig.cap="Summary of single species status for NEFMC stocks \\label{KOBE}", echo = F, out.width='300pt', message = FALSE, warning = FALSE}
...
```{r Commerical Landings, fig.cap="Commercial landings of A: Benthos, B: Mesoplanktivores, C: Macroplanktivores, D: Macrozoo-piscivores, E: Benthivores, F: Piscivores \\label{commland}", echo = F, fig.height=6}
...
```{r, fig.cap="Commercial revenue from A: Benthos, B: Mesoplanktivores, C: Macroplanktivores, D: Macrozoo-piscivores, E: Benthivores, F: Piscivores \\label{commrev}", echo = F, fig.height=6}
opar <- par(mfrow = c(6, 1), mar = c(0, 0, 0, 0), oma = c(4, 6, 2, 6))
...
soe.stacked.axis('Year', 'Commercial Revenue, million $US')
```
...
```{r, fig.cap="Fall survey trends for A: Benthos, B: Mesoplanktivores, C: Macroplanktivores, D: Macrozoo-piscivores, E: Benthivores, F: Piscivores \\label{fallsurvB}", echo = F, fig.height = 6}
opar <- par(mfrow = c(6, 1), mar = c(0, 0, 0, 0), oma = c(4, 6, 2, 6))
...
```{r ,fig.cap="Spring survey trends for A: Benthos, B: Mesoplanktivores, C: Macroplanktivores, D: Macrozoo-piscivores, E: Benthivores, F: Piscivores \\label{springsurvB}", echo = F, fig.height = 6}
opar <- par(mfrow = c(6, 1), mar = c(0, 0, 0, 0), oma = c(4, 6, 2, 6))
...
knitr::include_graphics(file.path(image.dir, 'GOMprod.pdf'))
knitr::include_graphics(file.path(image.dir, 'ProdLegend.pdf'))

```

```{r, fig.cap="Fish productivity: Anomalies of recruit abundance per spawner biomass for X species in the GOM. Annual anomalies shown are the average of spring and fall anomalies. \\label{fishprodsurvey}", fig.height=0.05}
opar<-par(mar=c(0,0,0,0))
plot(0,0,typ='n',axes=F,xlab='',ylab='')
...
```{r, fig.cap="Annual sea surface temperature cycle, 2016 \\label{annual cycle}" , echo=F, fig.show='hold', out.width='.45\\linewidth'}
...
```{r, fig.cap="Interpolated survey bottom temperature, A: April, B: October \\label{survtemp}",echo = F, out.width='.5\\linewidth', fig.show='hold'}
opar <- par(mfrow = c(2, 1), mar = c(0, 0, 0, 0), oma = c(4, 6, 2, 6))
...
```{r , fig.cap="Survey measured salinity; A: Surface, B: Bottom \\label{Salinity}", echo = F, out.width='.5\\linewidth', fig.show='hold'}
opar <- par(mfrow = c(2, 1), mar = c(0, 0, 0, 0), oma = c(4, 6, 2, 6))
