...
##Commercial Profits
This element is applied at the ecosystem level. This element addresses the risk of not maximizing fishery value. 
...
soe.plot(allrev, 'Time', 'MAFMC Revenue', x.label="Year", y.label="Commercial Revenue USD", rel.y.num = 1.1, rel.y.text = 1)
```

##Recreational Value
This element is applied at both the fleet level and at the ecosystem level where it would apply equally to all recreationally fished species. Risk of not maximizing fishery value is evaluated using the number of angler-days and number of trips in aggregate.
...
These significant recent decreases in numbers of anglers and numbers of trips alone suggest high risk to recreational value generated from the species with substantial recreational fisheries (summer flounder, scup, black sea bass, bluefish). However, this is a national trend likely due to shifting demographics and general economic dynamics, rather than an issue that can be managed at the MAFMC [@national_marine_fisheries_service_fisheries_2017].
...
```{r, fig.cap="Diversity in species revenue: A: all species, B: scallops and clams excluded \\label{fleetsppdiv}", echo = F, fig.height=3.5}
...
opar <- par(mfrow = c(2, 1), mar = c(0, 0, 0, 0), oma = c(4, 6, 2, 6))
soe.plot(altfleetdiv.data, 'Time', 'Mid-Atlantic commercial species diversity', x.end = 2016,
          rel.y.num = 0.8, stacked = 'A')
soe.plot(altfleetdiv.data, 'Time', 'Mid-Atlantic commercial species diversity no scallop clam', x.end = 2016,
          rel.y.num = 0.8, stacked = 'B')

soe.stacked.axis('Year', expression('Shannon index'),
                 rel.y.text = .8)
...
This index shows a significant long term decrease when all species are included in revenue, which would suggest a moderate-high risk to fishery business resilience based on diversity in species revenue. When scallops and clams are removed from species-based revenue, there is no long term trend, suggesting low risk to fishery business resilience. 
