---
title: "State of the Ecosystem - Georges Bank"
...
The purpose of this report is to provide **ecosystem-scale information for fishery managers** to consider along with existing species-scale analyses. An overview of ecosystem relationships as represented by **a conceptual model helps place more detailed species-level management in context** by highighting relationships between focal species groups, managed human activities, environmental drivers, habitats, and key ecological links. The activities link to high level strategic management objectives (described next). The model below is meant primarily to demonstrate the major linkages and extent of ecosystem relationships. Specific key paths through the ecosystem that are relevant to managers will be supplied in later discussions. Many components of the conceptual model are represented by indicators in this report.

##Conceptual model of ecosystem relationships
```{r, fig.cap="Georges Bank Ecosystem \\label{conceptual}", echo = F, out.width = "500pt", fig.align='center'}
knitr::include_graphics(file.path(image.dir, 'GB_2.png'))
...
```{r, fig.cap="Summary of single species status for MAFMC stocks \\label{KOBE}", echo = F, out.width='300pt', message = FALSE, warning = FALSE}
...
```{r Mariculture, fig.cap="Aquaculture stats from RI: total value (left), oysters sold (right) \\label{mariculture}", echo = F, out.width='.45\\linewidth', fig.show='hold'}

opar <- par(mfrow = c(1, 1), mar = c(0, 0, 0, 0), oma = c(4, 6, 2, 6))
soe.plot(SOE.data, 'Time', 'aquaculture RI total value value', x.label = 'Year', y.label='Total value $')

opar <- par(mfrow = c(1, 1), mar = c(0, 0, 0, 0), oma = c(4, 6, 2, 6))
soe.plot(SOE.data, 'Time', 'aquaculture RI oysters sold', x.label ='Year', y.label='Amount sold, number')
...
##Species composition 
Diversity in species composition mainly addresses objectives related to ecosystem structure and stability; maintaining diversity (here estimated as the mean number of species found in a random sample of 100 fish at a station for the Georges Bank portion of NEFSC surveys) can provide the capacity to adapt to change at the ecosystem level and for dependent fishing communities. 
...
Spatial distribution has changed over time for some species more than for others. Yellowtail flounder distributions measured by NEFSC surveys have shifted northward relative to historical distributions. However, haddock distributions on Georges Bank have remained relatively stable. A full suite of these maps is available at http://www.nefsc.noaa.gov/ecosys/current-conditions/kernel-density.html. 
...
```{r, fig.cap="Fish productivity: Anomalies of recruit abundance per spawner biomass for X species in the GB. Annual anomalies shown are the average of spring and fall anomalies. \\label{fishprodsurvey}", fig.height=0.05}
opar<-par(mar=c(0,0,0,0))
plot(0,0,typ='n',axes=F,xlab='',ylab='')
...
The Middle Atlantic Bight experienced above average sea surface temperatures (SSTs) during 2016. In the graph, the long term mean SST is shown as a dark gray line with areas representing plus and minus one and two standard deviations of the mean as progressive shades of gray, respectively. SSTs for 2016 that were above the mean are shown in red and below the mean in blue. Spring conditions moderated in the Mid-Atlantic Bight to below average temperature conditions in June and July. The ecoregions warmed dramatically from late summer into fall and remained above average temperatures through the end of the year.

```{r, fig.cap="Annual sea surface temperature cycle, 2016 \\label{annual cycle}" , echo=F, out.width='300pt'}
...
opar <- par(mfrow = c(2, 1), mar = c(0, 0, 0, 0), oma = c(4, 6, 2, 6))
soe.plot(SOE.data, 'Time', 'Surface temp GB', stacked = 'A', tol = 0.01, rel.y.num = 1.1)
soe.plot(SOE.data, 'Time', 'Bottom temp GB', stacked = 'B', tol = 0.01, rel.y.num = 1.1)  
...
Seasonal sea surface temperature forecasts are made from an ensemble of seven models over the period starting in February 2017 and ending in August 2017. There was relatively good model agreement in the forecasts for the Middle Atlantic Bight suggesting that sea surface temperature will rebound in the coming months to an ensemble mean of approximately 0.9°C above average. Skill of these forecasts for our specific region requires further evaluation. 
...
opar <- par(mar = c(4, 6, 2, 6))
soe.plot(SOE.data, 'Time', 'Stratification (0-50m) GB core', x.label = 'Year', rel.y.num = 1.2, rel.y.text = 1,
         y.label = 'Stratification', y.line = 4.7)
```
...
SSC recommendations will go here. 

# More Information  
http://www.nefsc.noaa.gov/ecosys/

# Acknowldegements  
The structure and framework of this report is based on best practices for Integrated Ecosystem Assessment (IEA) developed through the ICES Working Group for the Northwest Atlantic Regional Sea (WGNARS), a joint working group of the US and Canada. Funding and intellectual support from the NOAA NMFS Integrated Ecosystem Assessment Program was essential to the production of this report. Finally, we are grateful for the contributions of many workers throughout the Northeast US region that are represented by the indicators.
