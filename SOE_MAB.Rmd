...
```{r Legend Box function, echo = F}
reduce.leg.box <- function(x, reduce.x = 1, reduce.y = 1){
  a <- x$rect
  
  #Determine mid-points of existing box
  h.mid <- a$top - 0.5*a$h
  w.mid <- a$left + 0.5*a$w
  #Determine new distance from midpoint
  h.red <- 0.5 * a$h * reduce.y
  w.red <- 0.5 * a$w * reduce.x
  
  #draw new box
  rect(xleft  = w.mid - w.red, ytop    = h.mid + h.red, 
       xright = w.mid + w.red, ybottom = h.mid - h.red)
}
```
...
plot(0, 0, xlim = c(-77, -69), ylim = c(35, 43), xlab = '', ylab = '', axes = F,
     typ = 'n')
...
a <- legend('bottomright', 
            legend = c('Low', 'Moderate', 'Medium/High', 'High'), bty = 'n',
            pch = 21, pt.bg = c('light green', 'green', 'yellow', 'red'), 
            pt.cex = point.cex, cex = 1, y.intersp = 1)
...
```{r, fig.cap="Risks from sea level rise (A), reliance on climate-vulnerable species (B), and catch diversity (C) \\label{commrisk}", echo = F, fig.height = 3.8, fig.width = 9}
...
a <- legend('bottomright', 
            legend = c('Low', 'Moderate', 'Medium/High', 'High'), bty = 'n',
            pch = 21, pt.bg = c('light green', 'green', 'yellow', 'red'), 
            pt.cex = point.cex, cex = 1.5, y.intersp = 1)
...
for(i in 2:4){
    ...
    points(risk[SpeciesVul == 'Moderate' & ValueRank == i, list(PRIMARY_LO, PRIMARY_LA)],
           pch = 21, bg = 'yellow', cex = point.cex[i])
    points(risk[SpeciesVul == 'High/Very High' & ValueRank == i, list(PRIMARY_LO, PRIMARY_LA)],
           pch = 21, bg = 'red', cex = point.cex[i])
}

a <- legend('bottomright', 
            legend = c('Mixed', 'Low', 'Moderate', 'High/Very High', '', 
                       '< $1M', '$1M - $10M', '> $10M'), bty = 'n',
            pch = 21, pt.bg = c('white', 'green', 'yellow', 'red', rep('white', 4)), 
...
Species distribution on the NE Shelf can be characterized by the position in the ecosystem along an axis oriented from the southwest to the northeast, referred to as the along shelf distance. Along shelf distances range from 0 to 1360, which relates to positions along the axis from the origin in the southwest to the northeast in kilometer units. The mean along shelf distance for several MAFMC species by year is shown below.
...
soe.plot(SOE.data, 'Time', 'BLACK SEA BASS ASDIST', stacked = "A")
soe.plot(SOE.data, "Time", "SUMMER FLOUNDER ASDIST", stacked = "B")
soe.plot(SOE.data, "Time", "SCUP ASDIST", stacked = "C")
soe.plot(SOE.data, "Time", "BUTTERFISH ASDIST", stacked = "D")
soe.plot(SOE.data, "Time", "ATLANTIC MACKEREL ASDIST", stacked = "E")
soe.plot(SOE.data, "Time", "LONGFIN SQUID ASDIST", stacked = "F")
soe.plot(SOE.data, "Time", "NORTHERN SHORTFIN SQUID ASDIST", stacked = "G")
soe.stacked.axis('Year', 'Along shelf distance, km', rel.y.text = 1)

```

Spatial distribution has changed over time for some species more than for others. Black sea bass distributions measured by NEFSC surveys have shifted northward relative to historical distributions. In contrast, longfin squid distributions in the Mid Atlantic Bight have remained relatively stable. 
<!--\etwocol-->
...
Zooplankton surveys have been conducted since the 1970s and have been most consistently executed in the spring and fall seasons coinciding with the NEFSC bottom trawl survey. The time series of zooplankton biovolume suggest that overall zooplankton production has not changed over time. However, the dominant species of zooplankton in the Bight, *Centropages typicus* shows a seasonal shift in abundance, suggesting a change in timing of zooplankton reproductive cycles, which may be impacting fish species such as mackerel.

```{r , fig.cap="A: Centropages typicus spring, B: Centropages typicus fall \\label{Ctyp}", echo = F, fig.height=3.5}
opar <- par(mfrow = c(2, 1), mar = c(0, 0, 0, 0), oma = c(4, 6, 2, 6))
soe.plot(SOE.data, 'Time', 'Centropages typicus spring', stacked = 'A', tol = 0.03, rel.y.num = 1.1)
soe.plot(SOE.data, 'Time', 'Centropages typicus fall', stacked = 'B', tol = 0.03, rel.y.num = 1.1)

soe.stacked.axis('Year', expression('num per 100 m' ^ '3'), rel.y.text = 1.1)
```
