...
```{r fleet diversity NE, fig.cap = "New England commercial fleet diversity (A) and fleet count (B).",echo=F, message=FALSE, warning=FALSE, include=T, fig.width=7, fig.height=4, fig.align = 'center',fig.pos='H'}
opar <- par(mfrow = c(2, 1), mar = c(0, 0, 0, 0), oma = c(4, 6, 2, 6))
...
```{r revenue diversity NE,fig.cap = "New England species revenue diversity.", echo = F, message=F, include=T, warning=F, fig.width=7, fig.height=4, fig.align = 'center',fig.pos='H'}
opar <- par(mar = c(4, 6, 2, 6))
...
```{r recreation NE,fig.cap = "Recreational participation (A) and number of angler trips (B) in New England.", echo=F, message=FALSE, warning=FALSE, include=T, fig.width=8, fig.height=6, fig.align = 'center'}
opar <- par(mfrow = c(2, 1), mar = c(0, 0, 0, 0), oma = c(4, 6, 2, 6))
soe.plot(SOE.data.2018, "Time","North Atlantic Rec participation", stacked = "A",
         endshade = T, rel.y.num = 1.1, full.trend = T, scale.axis = 10^6, status = F,end.start = 2007, cex.stacked = 1.5)
...
```{r NE_HAB, fig.cap="HAB-related shellfish bed closures in New England (2007-2016).", echo=F,message=FALSE, warning=FALSE ,fig.align='center', fig.asp=1.1,fig.width=6,fig.pos='H'}
...
plot(coast, xlim = c(-71.5,-64.5), ylim = c(41.5,44.5),col = "grey")
...
axis(1, at = c(-71,-69,-67,-65), labels = paste( c(-71,-69,-67,-65) * -1, 'W'),col = NA, col.ticks = 1)
axis(2, at = axTicks(2), labels = paste(axTicks(2), 'N'), las = T, pos = -71.78,col = NA, col.ticks = 1)
box(lty = 1)
...
```{r sea_scallop_maps,fig.cap="Sea scallop historical and current abundance estimates (A), current thermal habitat estimate (B), and 20-40 year thermal habitat projection (C). \\label{dists}" , fig.width = 8,fig.height = 2.75, fig.show='hold',fig.pos='H'}
#read images
kde <-readPNG("sea_scallop_kde.png")
contemp <-readPNG("sea_scallop_contemp.png")
proj <- readPNG("sea_scallop_20_40.png")
...
```{r atlantic cod_maps,fig.cap="Atlantic cod historical and current abundance estimates (A), current thermal habitat estimate (B), and 20-40 year thermal habitat projection (C). \\label{dists}" , fig.width = 8,fig.height = 2.75, fig.show='hold',fig.pos='H'}
#read images
kde <-readPNG("atlantic_cod_kde.png")
contemp <-readPNG("atlantic_cod_contemp.png")
proj <- readPNG("atlantic_cod_20_40.png")
...
### SST anomaly maps
```{r sst_anomaly_maps, fig.cap="Sea surface temperature anomalies in Fall (A), fig.height=5.5, fig.pos='H', fig.show='hold', message=FALSE, warning=FALSE, Winter (B), Spring (C), and Summer (D) 2017. \\label{dists}" , fig.width=8}
if (map_figs == T){
...
soe.plot(SOE.data.2018, "Time", "Calanus finmarchicus abundance anomaly GOM", 
         anomaly = T, stacked = "A", cex.stacked = 1.5, full.trend = T, cex.stacked = 1.5)
