...
#STOP HERE
#Do you have the most updated version of the SOE.data.2018 dataset AND the necessary spatial data? Check the SOE_NE google drive for most recent version.
...
| E: **Benthivores** (Eat bottom dwellers)       |  25 | lobster, haddock, yellowtail, winter, and witch flounders,barnoor skate, ocean pout, black sea bass, scup, tilefish, tautog, cunner, blue crab, red crab, other crabs |
...
### Gulf of Maine
```{r seafood_landings_GOM,  fig.cap = "Total landings (black) and total landings managed by NEFMC (red) in Gulf of Maine (A: Apex predators, B: Piscivore, C: Planktivore, D: Benthivore, E: Benthos).",echo = F, warning = F, include = T,out.width='1\\linewidth', fig.width = 6,fig.height=7.5, fig.align='center',fig.pos='H'}
...
```
### Georges Bank
```{r seafood_landings_GB,fig.cap="Total landings (black) and total landings managed by NEFMC (red) in Georges Bank (A: Apex predators, B: Piscivore, C: Planktivore, D: Benthivore, E: Benthos).", echo = F, warning = F, include = T,out.width='1\\linewidth', fig.width = 6,fig.height=7.5, fig.align='center',fig.pos='H'}
...
```{r managed_landings_NE , fig.cap = "Total landings (black) and total landings managed by NEFMC (red) in Gulf of Maine (A) and Georges Bank (B).", echo=F, message=FALSE, warning=FALSE , fig.width=7, fig.height=4, fig.align = 'center'}
...
```{r rec_catch_NE, fig.cap = "Recreational seafood landings in New England.",echo = F, message=F, include=T, warning=F, fig.width=7, fig.height=4, fig.align = 'center',fig.pos='H'}
...
```{r 'Rev_Price_Vol_GOM_GB',fig.cap = "Revenue change in Gulf of Maine (A) and Georges Banks (B) shown through aggregated price and volume indicators." , fig.width = 8,fig.height = 2.75, fig.show='hold',fig.pos='H'}
price_ind<-readPNG("Pr_Vol_GOM.png")
vol_ind<-readPNG("Pr_Vol_GB.png")
...
```{r 'price_ind_GOM',fig.cap="Bennet indicator, price component (A) and volume component (B) by functional group\\label{Bennetspp1}." , fig.width = 8,fig.height = 2.75, fig.show='hold',fig.pos='H'}
price_ind<-readPNG("GOM_PI.png")
vol_ind<-readPNG("GOM_VI.png")
...
```{r 'price_ind_GB',fig.cap="Bennet indicator, price component (A) and volume component (B) by functional group\\label{Bennetspp2}." , fig.width = 8,fig.height = 2.75, fig.show='hold',fig.pos='H'}
price_ind<-readPNG("GB_PI.png")
vol_ind<-readPNG("GB_VI.png")
...
```{r managed_revenue_NE , fig.cap="Total revenue by region and MAFMC managed species\\label{totcommrev}.", echo=F, message=FALSE, warning=FALSE , fig.width=7, fig.height=4, fig.align = 'center',fig.pos='H'}
opar <- par(mfrow = c(2, 1), mar = c(0, 0, 0, 0), oma = c(4, 6, 2, 6))
...
soe.stacked.axis('Year', 'Fleet diversity', outer = F,rel.y.text = 0.9, y.line = 3)
soe.plot(SOE.data.2018,"Time","New England fleet count", stacked = "B",
         endshade = T, rel.y.num = 1.1, full.trend = F, status = F,end.start = 2007, cex.stacked = 1.5)
soe.stacked.axis('Year', 'Fleet count', outer = F,rel.y.text = 0.9, y.line = 3)
```

## New England revenue diversity

```{r revenue diversity NE,fig.cap = "New England species revenue diversity.", echo = F, message=F, include=T, warning=F,  fig.asp=0.5, fig.align = 'center',fig.pos='H'}
opar <- par(mar = c(4, 6, 2, 6))
...
soe.plot(SOE.data.2018,"Time","North Atlantic angler trips", stacked = "B",
         endshade = T, rel.y.num = 0.9, full.trend = T, scale.axis = 10^6, status = F,end.start = 2007, cex.stacked = 1.5)
soe.stacked.axis('Year', expression('Recreational Participation, 10'^6*'n'), rel.y.text = 0.9)
```



```{r NE_HAB, fig.cap="HAB-related shellfish bed closures in New England (2007-2016).", echo=F,message=FALSE, warning=FALSE ,fig.align='center', fig.asp=1.1,fig.width=6,fig.pos='H',strip.white=T}
#get map data
e  <- extent(-78.5, -64, 41, 45)
coast1 <- crop(coast, e)

events <- SOE.data.2018[grepl("occurrence NE",SOE.data.2018$Var),]

events_df <- data.frame(lon = events[grepl("Lon",events$Var),]$Value,
                        lat = events[grepl("Lat",events$Var),]$Value,
                        val = events[grepl("NE N",events$Var),]$Value,
                        var = c("DSP","ASP","ASP",rep("PSP",5)))
...
plot(g2,  add = T,cex = 4, pch = 16,
     col = c("grey", "purple","darkorange","darkorange","darkorange"))
plot(g3,  add = T,cex = 2, pch = 16, col = c("purple","darkorange"))
...
legend(-66.2,42.5, c("PSP", "ASP", "DSP"), col = c("darkorange","purple","grey"), pch = 16,
       cex = 1.1, bty = "n", pt.cex = 2)
text(-65.55,42.495, "Category")
...
```{r seas_scallop_maps,fig.cap="Sea scallop historical and current abundance estimates (A), current thermal habitat estimate (B), and 20-40 year thermal habitat projection (C). \\label{dists}" , fig.width = 8,fig.height = 2.75, fig.show='hold',fig.pos='H', warning=F, message=F}
#read images
kde <-readPNG("sea_scallop_kde.png")
...
text(.12,.9, "A", cex=1.5)
th_plot(data = "Sea scallopfall_2.nc", species = "sea scallop", letter = "B", z.max = 2.855913)
th_plot(data = "Sea scallopfall_4.nc", species = "sea scallop", letter = "C", z.max = 2.855913)
...
```{r atlantic cod_maps, echo=FALSE, fig.cap="Atlantic cod historical and current abundance estimates (A), current thermal habitat estimate (B), and 20-40 year thermal habitat projection (C). \\label{dists}", fig.height=2.75, fig.pos='H', fig.show='hold',warning=F, message=F,  fig.width=8}
#read image
kde <-readPNG("atlantic_cod_kde.png")
...
rasterImage(kde, usr[1], usr[3], usr[2], usr[4])
text(.12,.9, "A", cex=1.5)
th_plot(data = "Atlantic codfall_2.nc", species = "atlantic cod", letter = "B", z.max = 5.190186)
th_plot(data = "Atlantic codfall_4.nc", species = "atlantic cod", letter = "C", z.max = 5.190186)
...
### SST anomaly maps
```{r sst_anomaly_maps, fig.cap="Sea surface temperature anomalies in Fall (A), Winter (B), Spring (C), and Summer (D) 2017.\\label{dists}", fig.height=5.5, fig.pos='H', fig.show='hold', message=FALSE, warning=FALSE , fig.width=8}
if (map_figs == T){
SST_plot <- function(raster_data, text){
  d <- raster(raster_data, varname = "ANOM")
  e  <- extent(-78.5, -64, 33, 46)
  d <- crop(d, e)
  ...
  par(mar = c(2,2,3.2,1))  
  plot(d, col=colors(n=color_levels),xlim = c(-78,-64),ylim = c(34,45),
  ...
  contour(bathy,xlim = c(-78,-65),drawlabels = F,nlevels = 5,add=T)
  ...
  arrows(-78.71,35,-65.75,35,angle = 180,lwd = 2.5)
  text(-77.75,44.5,text,cex = 1.5)
}
...
```{r calfin_anom_NE,fig.cap = "\\textit{C. finmarchicus} abundance anomaly in Gulf of Maine (A) and Georges Bank (B)",echo = F, warning = F, include = T,  fig.show='hold', fig.align='default', out.width='.49\\linewidth',warning = F, message = F}

#C. finmarchicus abundance anomaly
opar <- par(mar = c(4,4.75,2,4))
soe.plot(SOE.data.2018, "Time", "Calanus finmarchicus abundance anomaly GOM", 
         anomaly = T, stacked = "A", cex.stacked = 1.5, full.trend = T)
soe.stacked.axis("Year", "Abundance anomaly", rel.x.text = 1.2, rel.y.text = 1.2, 
                 y.line = 3.2, outer = F)

opar <- par(mar = c(4,4.75,2,4))
soe.plot(SOE.data.2018, "Time", "Calanus finmarchicus abundance anomaly GB", 
         anomaly = T, stacked = "B", suppressAxis = T, cex.stacked = 1.5, full.trend = T)
soe.stacked.axis("Year", "Abundance anomaly", rel.x.text = 1.2, rel.y.text = 1.2, 
                 y.line = 3.1, outer = F)
```

```{r, NE zooplankton, fig.cap="\\textit{C. typicus} abundance anomaly (A), and small-large copepod index with primary productivity anomaly (B) in the Mid-Atlantic.",echo = F, warning = F, include = T,  fig.show='hold', fig.align='default', warning = F, message = F, fig.height = 6.2, fig.width=5}
