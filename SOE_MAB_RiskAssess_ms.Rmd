...
```{r riskel, tab.cap="Risk Elements, Risk Definitions, and Indicators Used\\label{riskel}", echo=FALSE, message=FALSE, warnings=FALSE, results='asis'}
tabl <- "
...
Risk criteria for each element are described in individual sections below, and summarized in Table \ref{allcriteria}. 

```{r allcriteria, tab.cap="Risk Ranking Criteria used for each Risk Element\\label{allcriteria}", echo=FALSE, message=FALSE, warnings=FALSE, results='asis'}

all<-read.table("riskrankingcriteria.txt", sep="|", header=T, strip.white = T)
names(all) <- c("Element", "Ranking", "Criteria")
allwide <- all %>% spread(Ranking, Criteria)
kable(allwide, format = "latex", booktabs = T)
...
Risk assessment will be used to lead into other aspects of IEA loop. In the Mid, could use the risk assessment to prioritize. Three ways, examples from WGNARS.
