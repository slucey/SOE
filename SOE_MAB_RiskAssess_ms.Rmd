...
risk.sppsector %>%
  ...
  kable(risk.sppsector, format="latex", escape = F, booktabs = T, linesep = "",
        caption="Species and sector (C=commerical, R=recreational) level risk analysis results; l=low risk (green), lm= low-moderate risk (yellow), mh=moderate to high risk (orange), h=high risk (red)\\label{spsectable}") %>%
...
cat(c("\\begin{table}[h] \\centering ", 
      "\\caption{Number of communities at each level of commercial (left) and recreational (right) reliance. ME=Maine, NH=New Hampshire, MA=Massachusetts, RI=Rhode Island, CT=Connecticut, NY=New York, NJ=New Jersey, PA=Pennsylvania, DE=Delaware, MD=Maryland, VA=Virginia, and NC=North Carolina.\\label{reliance}}",
      t1,
      "\\hspace{1cm} \\centering ",
      t2,
      "\\end{table}"))  
...
kable(commdat, format = 'latex', booktabs = T, caption = "Selected Mid-Atlantic Fishing Communities with Medium High to High Dependence on both Commercial and Recreational Fishing. ME=Maine, NH=New Hampshire, MA=Massachusetts, RI=Rhode Island, CT=Connecticut, NY=New York, NJ=New Jersey, PA=Pennsylvania, DE=Delaware, MD=Maryland, VA=Virginia, and NC=North Carolina. \\label{community}") %>%
