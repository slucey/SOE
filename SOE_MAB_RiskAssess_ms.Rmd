...
#Introduction
Fishery managers worldwide are evaluating methods for incorporating climate, habitat, ecological, social, and economic factors into current operations in order to implement Ecosystem Approaches to Fishery Management (EAFM). While this can seem overwhelming, it is possible to take practical steps towards EAFM implementation that make use of existing information and provide managers with valuable strategic advice. Here, we describe the process used by the U.S. Mid-Atlantic Fishery Management Council (Council) to develop an ecosystem-level risk assessment, the initial step proposed in their recently adopted EAFM policy guidance. 
...
• December 1 EOP Committee-initial review of “ranking criteria” matrices (applying data to rank within each individual element, not across elements). Updates to ranking criteria

*What is the risk?* The **Risk Ranking Criteria** were developed iteratively between analysts and the Council to use the available indicator(s).  We note that for many of these there is no established high risk threshold, so the consensus of the Council and committee were used to establish risk levels. If clear thresholds become available for any element/indicator combination, they can be substituted into future assessments. Similarly, if the risk tolerance of the Council changes over time, the ranking criteria can be similarly adjusted. 


• December 2017 Council Meeting: Council initial agreement on ranking criteria of elements, results of risk assessment based on those criteria


As a result of this process, the Council finalized a range of risk elements to be evaluated at either the managed species level, the species and sector level, or the ecosystem level. An overview of the risk elements with definitions and associated indicators as adopted by the MAFMC is presented in as the first result of this assessment. In the following results sections, we describe each risk element in more detail along with Council-specified definitions of low, low-moderate, moderate-high, and high risk. Indicators are then shown for each risk element and a risk categorization based on the indicator is presented. For trend-based risk definitions, a Mann-Kendall test for monotonic trends is used to test significance (p<0.05) of both long term and recent trends. Autocorrelation in the time series was addressed by prewhitening the data as suggested by [@yue_influence_2002]. At the end of the results section, we summarize risk ranking results across elements in three tables.
...
The final risk ranking criteria agreed on by the Council for each element are described in the individual sections below, and summarized for all risk elements in Table \ref{allcriteria}. Each section also presents the indicator data for the element and describes risk based on the risk ranking criteria. Summary results across all risk categories are presented in Table \ref{sptable} (species level), Table \ref{spsectable} (species and sector level), and Table \ref{ecotable} (ecosystem level). 

## Ecological Elements

###Assessment Performance
This element is applied at the species level. Risk elements below (F status and B status) describe risks according to our best understanding of stock status, but assessment methods and data quality shape our understanding. This risk element addresses risk to achieving OY due to scientific uncertainty based on analytical limitations. The MAFMC risk policy accounts for scientific uncertainty in assessments, with methods for determining scientific uncertainty currently being refined by the Scientific and Statistical Committee. Ranking for this risk element will be adjusted if necessary to ensure consistency with SSC methods in the future. 
<!--
...
This is aggregate commercial revenue for MAFMC managed species (Fig. \ref{econinds}, Upper left). There is a long term significant decrease in revenue, indicating moderate-high risk to commercial fishery profit. This trend is consistent with the trend first shown in the EAFM Interactions white paper and published in @gaichas_framework_2016 (Figs 2-3).
<!--
...
This is commercial seafood landings from MAFMC managed species (Fig. \ref{seafood}, Left). Because this is total landings, years prior to 1977 do include foreign landings (in particular, of Atlantic mackerel, which account for much of the observed spike). Recent landings are all domestic fisheries. Looking across all regions, there is a significant recent decrease in seafood landings, indicating high risk to regional domestic seafood production. 
<!--
...
This is total recreational harvest (all species) in the Mid-Atlantic region (Fig. \ref{seafood}, Right).
<!--
