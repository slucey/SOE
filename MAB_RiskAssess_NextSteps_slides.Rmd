...
- The risk assessment was designed help the Council decide where to focus limited resources to address ecosystem considerations by first clarifying priorities. 
...
## Prioritization
- One potential prioritization approach focuses on the single species with the largest number of high risk rankings across all elements, which is black sea bass (8 high Risk Elements<!--, Tables \ref{sptable}-\ref{ecotable}-->). 
- A second approach focuses on the fishery with the highest landings value (as a proxy for seafood production and economic benefits), which is the squid complex, with uncertainties in assessment and potential for distribution shifts along with high risks identified across multiple management elements <!--(Tables \ref{sptable}-\ref{spsectable})-->. 
- A third approach focuses on the risk element with the most high risk categorizations across all categories, which is allocation (12 high risk species/sector combinations<!--; Table \ref{spsectable}-->). 
...
- Developed by modifying the existing Mid-Atlantic conceptual model using the risk assessment results tables to ensure that all key factors affecting black sea bass and its fisheries were considered. 
- Will now step through this process briefly during the EOP presentation to illustrate methods for building a model. 
- While it looks complex, this need not be time consuming and the process can incorporate the knowledge of managers, fishery participants, and scientsts to ensure that key interactions and management priorities are considered and accounted for. 

## Conceptual model: example 1
```{r, fig.cap="Example conceptual model for a species determined to be high risk: black sea bass. \\label{bsbex}", fig.align='center', out.width="\\linewidth", echo=F, message=F}
...
## Conceptual model step 5: managment components
```{r,  fig.align='center', out.width="\\linewidth", echo=F, message=F}
...
## Conceptual model: example 2
```{r, fig.cap="Example conceptual model for a fishery determined to be high risk: longfin squid. \\label{squidex}", fig.align='center', out.width="\\linewidth" , echo=F, message=F}
...
## Conceptual model --> Managment Strategy Evaluation
...
- Any MSE undertaken by the Council will be a deliberative process and will include a variety of stakeholder and Council engagement/involvement. 
...
- EAFM potentially expands the scope of interactions and uncertainties to consider in evaluating harvest control rules and other fishery management procedures. 
...
- A risk matrix, such as the one developed by the Council, can be used to quickly evaluate where further integrated analysis and MSE should be focused--which fishery management plans, which species, and which Risk Elements need to be included in the analysis. 
- Further, because the risk assessment includes social and economic elements as well as the more standard ecological elements, the conceptual model and resulting MSE are designed to include the relevant linkages and therefore avoid unintended consequences of management actions <!--[@degnbol_unintended_2007]-->. 
